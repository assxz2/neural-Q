...
from rouran.landmark_estimation.model_utils import batch_with_heatmap
...
def train_epoch(config, train_loader, model, critertion, optimizer,
                epoch, writer_dict):
    ...
    for i, (inputs, target, meta) in enumerate(train_loader):
        ...
        if writer_dict:
            ...
            # select first image in score_map compare with the estimations
            # gt_batch_img = batch_with_heatmap(inputs.detach(), target.detach())
            # pred_batch_img = batch_with_heatmap(inputs.detach(), output.detach())
            # writer.add_image('score_map/prediction', pred_batch_img, global_steps)
            # writer.add_image('score_map/groundtruth', gt_batch_img, global_steps)
